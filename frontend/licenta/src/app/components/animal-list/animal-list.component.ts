...
export class AnimalListComponent implements OnInit {
  ...
  constructor(
    ...
    private reviewService: AnimalService,
  ...
  hydrateAllReviews() {
    if (this.universityId && this.userId) {
      ...
      this.reviewService
        .getAnimalsByAnimalCenterIdAndAuthorId(this.universityId, this.userId)
        .subscribe({
          next: (data) => {
      ...
      this.reviewService.getAnimalsByAnimalCenterId(this.universityId).subscribe({
        next: (data) => {
          this.allReviews = data;
        },
        error: (_) => { },
      ...
      this.reviewService.getAnimalsByAuthorId(this.userId).subscribe({
        next: (data) => {
          this.allReviews = data;
        },
        error: (_) => { },
      ...
      // Get all reviews
      this.reviewService.getAllAnimals().subscribe({
        next: (data) => {
          this.allReviews = data;
        },
        error: (_) => { },
  ...
  ngOnInit(): void {
    ...
    this.universityId =
      JSON.parse(this.route.snapshot.paramMap.get('animalCenterId') || 'null') ||
      undefined;
    ...
    this.university = this.universityId
      ? this.universityService.getAnimalCenterById(this.universityId).subscribe({
        next: (data) => {
    ...
    // Get own reviews
    this.reviewService.getAnimalsByAuthorId(this.user.id).subscribe({
      next: (data) => {
  ...
  public goToAddReview() {
    ...
    if (user_role == 'ADMIN')
      this.router
        .navigate([
          '/admin/animal-centers-animals/add',
      ...
      this.router
        .navigate([
          '/user-dashboard/animal-centers-animals/add',
          { universityId: this.universityId },
        ])
        .then((_) => { });
  }
  ...
  public deleteReview(review: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.reviewService.deleteAnimalById(review.id).subscribe({
          next: (_) => {
